...
//Colocar ! significa que ele é obrigatório (ex: tem que ser string, não pode ser int)
const typeDefs = `
    type Livro {
        id: ID!
        titulo: String!
...
const resolvers = {
    Query: {
        bemVindo(parent, args, ctx, info){
            ...
            // return "Bem vindo!"
            //se o clinete enviar o seu nome (joão), devolver Hello, João
        ...
        adicionar(parent, args, ctx, info){
            // ? = se - : = senão
            return args.numeros.length <= 0 ? 0 : args.numeros.reduce((ac, atual) => {
                return ac + atual
            })
        },
